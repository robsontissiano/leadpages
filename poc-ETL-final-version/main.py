...
class TransformedAnimal(BaseModel):
    ...
    born_at: Optional[
        str
    ]  # Must be a string in ISO8601 format for correct serialization
...
async def retry_fetch_page(page: int):
    ...
    async with httpx.AsyncClient() as client:
        while retries < MAX_RETRIES:
            try:
                logging.info(
                    f"Retrying fetching animals on page {page} (Attempt {retries + 1})"
                )
                response = await client.get(
                    f"{BASE_URL}/animals", params={"page": page}
                )
                response.raise_for_status()
                data = response.json()
                logging.info(
                    f"Successfully fetched animals on page {page} after {retries + 1} retries."
                )
                ...
                if hasattr(exc, "response") and exc.response is not None:
                    if exc.response.status_code in [500, 502, 503, 504]:
                        ...
                        wait_time = random.randint(
                            RETRY_WAIT_TIME[0], RETRY_WAIT_TIME[1]
                        )
                        logging.warning(
                            f"Error on page {page} with status {exc.response.status_code}. Retrying in {wait_time} seconds."
                        )
                        await asyncio.sleep(wait_time)
                    else:
                        logging.error(
                            f"Unrecoverable error fetching page {page}. Skipping..."
                        )
                        return []
                else:
                    logging.error(
                        f"Failed to fetch animals on page {page} after {retries} retries."
                    )
    ...
    async with httpx.AsyncClient() as client:
        while True:
            try:
                ...
                logging.error(
                    f"Error occurred while fetching animals on page {page}: {exc}"
                )
                logging.info(
                    f"Continuing to the next page ({page + 1}) while retrying page {page} in the background."
                )
    ...
    async with httpx.AsyncClient() as client:
        while retries < MAX_RETRIES:
            try:
                ...
                return AnimalDetail(
                    **response.json()
                )  # Return the animal detail if successful
                ...
                if isinstance(exc, httpx.ReadTimeout):
                    logging.error(
                        f"ReadTimeout occurred while fetching animal ID {animal_id}: {exc}"
                    )
                else:
                ...
                if hasattr(exc, "response") and exc.response is not None:
                    ...
                    if status_code in [500, 502, 503, 504]:
                        ...
                        logging.warning(
                            f"Error fetching animal ID {animal_id} with status {status_code}. Retrying in {wait_time} seconds (Attempt {retries + 1}/{MAX_RETRIES})"
                        )
                        await asyncio.sleep(wait_time)
                    else:
                        logging.error(
                            f"Failed to fetch details for animal ID {animal_id} after {retries} retries."
                        )
                        logging.error(traceback.format_exc())
                        return None
                else:
                    logging.error(
                        f"Unrecoverable error or no response available for animal ID {animal_id}. Details: {traceback.format_exc()}"
                    )
                    return None
...
def transform_animal(animal_detail: AnimalDetail) -> TransformedAnimal:
    ...
    if animal_detail.born_at:
        born_at_utc = (
            datetime.datetime.utcfromtimestamp(animal_detail.born_at / 1000).isoformat()
            + "Z"
        )
        ...
        while retries < MAX_RETRIES:
            try:
                ...
                response = await client.post(
                    f"{BASE_URL}/home", json=json_data
                )  # Fix: Use "json=" for async requests
                response.raise_for_status()
                ...
                if exc.response.status_code == 422:
                    logging.error(
                        f"Unprocessable Entity (422) error: {exc.response.text}"
                    )
                    break
                    ...
                    if retries < MAX_RETRIES:
                        ...
                        logging.warning(
                            f"Server error ({exc.response.status_code}). Retrying in {wait_time} seconds (Attempt {retries + 1}/{MAX_RETRIES})"
                        )
                        await asyncio.sleep(wait_time)
                    else:
                        logging.error(
                            f"Exceeded maximum retries for batch. Offloading retry to a separate thread."
                        )
                        executor.submit(retry_failed_post, animals_batch)
                        return
            except Exception as e:
                logging.error(
                    f"Failed to post batch after {retries} retries. Exception: {e}"
                )
                logging.error(traceback.format_exc())
                return
...
async def process_animals_in_batches(all_animals):
    ...
    for animal in all_animals:
        ...
        if len(tasks) % MAX_BATCH_SIZE == 0:
            ...
            transformed_batch = [
                transform_animal(detail) for detail in animal_details if detail
            ]
